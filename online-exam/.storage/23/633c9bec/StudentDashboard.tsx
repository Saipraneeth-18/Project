...
import { subjects, examSchedules, ExamSchedule } from '@/lib/examData';
import { Clock, Calendar, BookOpen, Trophy, LogOut, AlertCircle, CheckCircle } from 'lucide-react';

interface ExamWithSchedule {
  id: string;
  name: string;
  year: number;
  duration: number;
...
export default function StudentDashboard() {
  ...
  const getAvailableExams = (): ExamWithSchedule[] => {
    if (!user?.year) return [];
  ...
  const isExamTimeValid = (schedule: ExamSchedule) => {
    const now = new Date();
